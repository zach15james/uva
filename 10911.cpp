// manually copied for typing practice from CP4 book
...
double dp(int mask) // DP state = mask
{
    ...
    ans = 1e9; // int with a large value 
    int two_pow_p1 = LSOne(mask); // speedup 
    int p1 = __builtin_ctz(two_pow_p1); // p1 is first on bit (what this does: )
    int m = mask - two_pow_p1; // turn off bit p2
    
    while(m)
    {
        int two_pow_p2 = LSOne(m); // then try to match p1
        int p2 = __builtin_ctz(two_pow_p2); // with another bit on p2
...
int main()
{
    ...
    while(scanf("%d", &N), N) // comment was: 'yes, we can do this :)... this being: 
    {
        for(int i = 0; i < 2*N; ++i)
